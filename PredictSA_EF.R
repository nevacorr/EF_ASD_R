...
write.csv(final_df, "finaL_df_for_xgboost.csv", row.names = FALSE)
...
ggplot(df_results, aes(x = Actual, y = Predicted)) +
  ...
  theme_minimal() +
  labs(title = paste("Actual vs. Predicted", response, "based on volumetric data and sex"),
...
df_cv_results <- data.frame(matrix(ncol = 2, nrow = 302))
df_cv_results$y = y
df_cv_results$pred = cv_results$pred

ggplot(df_cv_results, aes(x = y, y = pred)) +. #not clear if pred order is the same 
  geom_point(color = "blue", alpha = 0.6) +
  geom_abline(slope = 1, intercept = 0, color = "red") +
  theme_minimal() 
  # labs(title = paste("Actual vs. Predicted", response, "based on volumetric data and sex"),
